...
export class TestComponent implements AfterViewInit, OnInit {
  ...
  test_nombre_quality(container: any, identiant: string, test: any) {
    ...
    if (elt.length <= 4) { // si le npmbre de qualite dans le card est a 3
      ...
      alert.innerHTML = `3 elements for each type <svg xmlns="http://www.w3.org/2000/svg" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
  ...
  refreshActivities() {
    ...
    this.service.getProfessions1().subscribe((res) => {
      this.professions1 = res;
    })
    this.service.getProfessions2().subscribe((res) => {
      this.professions2 = res;
    })
    this.service.getProfessions3().subscribe((res) => {
      this.professions3 = res;
    })
    this.service.getProfessions4().subscribe((res) => {
      this.professions4 = res;
    })
  }
